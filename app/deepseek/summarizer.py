"""
Long context summarizer via external OpenAI-compatible API.
"""
...
async def _call_once(api_url: str, api_key: str, model: str, text: str, timeout_s: float) -> str:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        resp = await client.post(api_url.rstrip("/") + "/chat/completions", headers=headers, json=payload)
    if resp.status_code != 200:
        raise SummarizerUnavailableError(f"Summarizer upstream returned status {resp.status_code}")
    try:
    ...
    *,
    ...
    for candidate in [model, *(fallbacks or [])]:
        try:
            ...
            last_error = exc
            logger.warning(f"Summarizer model '{candidate}' failed: {exc}")
            continue
    raise SummarizerUnavailableError(f"All summarizer models failed: {last_error}") from last_error
